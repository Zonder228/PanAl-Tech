...
## Модуль Искусственного Интеллекта (Мультимодальная нейронная сеть)

### Почему выбрана архитектура Late Fusion (позднего слияния)

Мы выбрали **архитектуру позднего слияния (Late Fusion)**, потому что четыре модальности данных существенно различаются по физической природе, размерности и характеру шума: RGB-изображения, геометрические признаки поверхности, FFT-спектры акустических сигналов и скалярные показания влажности.

Late Fusion позволяет каждой ветке максимально эффективно обучаться независимо, использовать наиболее подходящую архитектуру и предобученные веса. Финальное слияние происходит на уровне высокоуровневых признаков. Такой подход обеспечил значительный прирост качества по сравнению с одиночным визуальным каналом, особенно при выявлении внутренних скрытых дефектов.
...
Модель обучается на комбинации датасетов **SDNET2018** и **Concrete Crack Images for Classification (Mendeley Data)** с применением аугментаций.

### Геометрический анализ (Depth Features)

**Архитектура:** многослойная полносвязная сеть (MLP).  
**Задача:** анализ рельефа поверхности, выявление локальных деформаций, выпуклостей и впадин.
...
class GeometryBranch(nn.Module):
    def __init__(self, input_dim: int = 32, feature_dim: int = 64):
        super().__init__()
...
**Архитектура:** 1D-CNN.  
**Задача:** выявление внутренних дефектов (пустоты, расслоения, коррозия арматуры) по спектральному отпечатку.
...
class AcousticBranch(nn.Module):
    ...
    def forward(self, fft_spectrum: torch.Tensor) -> torch.Tensor:
...
class HumidityBranch(nn.Module):
    ...
    def forward(self, humidity_value: torch.Tensor) -> torch.Tensor:
        return self.mlp(humidity_value.unsqueeze(1))
```

**Примечание:** ветка влажности обучалась преимущественно на **синтетических данных**.
...
### Почему мы использовали именно эти модальности

Визуальный осмотр (даже с помощью камер) хорошо справляется только с поверхностными трещинами, но практически **не способен** обнаруживать внутренние пустоты, расслоения (деламинацию), коррозию арматуры под слоем бетона и зоны повышенной влажности.

- **Геометрический канал** дополняет визуальный анализ информацией о рельефе поверхности.
- **Ударно-акустический канал** позволяет «слышать» внутренние дефекты по изменению спектра.
- **Канал влажности** служит важным косвенным индикатором деградации конструкции.

Сравнение с чисто визуальным baseline наглядно демонстрирует ценность мультимодального подхода и его превосходство над традиционными методами обследования.
